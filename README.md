...
- **点击间隔可调**：1ms ~ 1000ms 整数区间，上下箭头一按走 5；也可以直接填「每秒点击次数」，两个输入框双向联动
...
- 连点期间每 50ms 用 `GetAsyncKeyState` 核对热键是否真的还按着。钩子收不到管理员进程的按键，那条「松开」消息可能永远不到，没有这道兜底程序会一直点下去。这项核对要等连点开始 50ms 之后才生效：低级键盘钩子跑在系统更新按键状态之前，热键刚按下的那一瞬间 `GetAsyncKeyState` 仍然返回「松开」，此时核对只会把自己刚启动的连点否掉
